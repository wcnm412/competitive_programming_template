...
// returns the maximum value of f(x) in the range [low, high]
int maximise(int low, int high) {
  int mid = (low + high) / 2;

  if (mid == low || mid == high) return max(f(mid), f(mid + 1));

  long long d1 = f(mid);
  long long d2 = f(mid+1);

  if (d1 > d2) {
    return maximise(low, mid);
  } else {
    return maximise(mid, high);
  }
