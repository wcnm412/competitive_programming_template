...
//Djikstra find shortest paths
map<int, vi> graph;
map<int, vi> inv;
map<int, map<int, int>> weights;

vvi allShortestPaths(int start, int end) {
  // djikstra
  vi dist(200005, INF);
  dist[start] = 0;
  ...
  while (!pq.empty()) {
    ...
    trav(nextNode, graph[currNode]) {
      int nextDist = currDist + weights[currNode][nextNode];
  ...
  while (!q.empty()) {
    ...
    if (currNode == start) {
      reverse(all(currPath));
      paths.push_back(currPath);
    }
    trav(nextNode, inv[currNode]) {
      if (dist[nextNode] + weights[nextNode][currNode] == dist[currNode]) {
        vi newPath = currPath;
